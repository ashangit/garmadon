...
public final class ElasticSearchListener implements BulkProcessor.Listener {
    ...
    public void afterBulk(long executionId, BulkRequest request, BulkResponse response) {
        ...
        CommittableOffset<String, byte[]> lastOffset = (CommittableOffset<String, byte[]>) 
            ((GarmadonIndexRequest) request.requests().get(request.requests().size() - 1)).getCommittableOffset();
        lastOffset
                .commitAsync()
