...
//        conf.set(MiniDFSCluster.HDFS_MINIDFS_BASEDIR, hdfsTemp.toString());
//        MiniDFSCluster.Builder builder = new MiniDFSCluster.Builder(conf)
//            .simulatedCapacities(new long[] {10240000L});
//        miniDFS = builder.build();

        HiveConf hiveConf = new HiveConf();
        //System.setProperty(HiveConf.ConfVars.METASTOREWAREHOUSE.toString(), derbyDBPath.toString());
        hiveConf.set(HiveConf.ConfVars.METASTORECONNECTURLKEY.toString(), "jdbc:derby:;databaseName=" + derbyDBPath.toString() + "/derbyDB" + ";create=true");
    ...
    public void connectAndCreateDatabaseWithoutIssue() throws SQLException {
        ...
        assertTrue(hiveClient.execute("DESCRIBE DATABASE " + database));
        try {
            assertFalse(hiveClient.execute("DESCRIBE DATABASE not_created_database"));
    ...
    public void createTableWithoutIssue() throws SQLException {
        ...
        try {
            assertFalse(hiveClient.execute("DESCRIBE FORMATTED not_a_table"));
        } catch (SQLException e) {
    ...
    @Test
    public void shouldReconnectIfNetFailure() throws Exception {
